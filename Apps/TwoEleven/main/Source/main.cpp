    ...
    while (!should_close) {
        ...
        switch (event.type) {
            ...
            case APP_EVENT_RESULT: {
                ...
                // Don't manipulate LVGL objects here - they may be invalid. Just store state for
                // twoElevenCreateWidgets to handle once it runs again.
                if (launch_id == ctx.selectionDialogId && ctx.selectionDialogId != 0) {
                    ...
                    // twoElevenCreateWidgets' resurface handling start a fresh game at the same
                    // grid size (that path is only for burial by something else entirely).
                    ctx.pendingSelection = -1;
