...
//
//    Adapted for the JUCE plugin build to read from an in-memory buffer
//    (via memio) rather than from a FILE on disk, so that MUS/MIDI lumps
//    can be decoded without touching the filesystem.
//
...
// MIDI files are big-endian; the local i_swap.h only provides little-endian
// helpers, so define the byte-reversing versions here (all target platforms
// are little-endian).

#ifndef SDL_SwapBE16
#define SDL_SwapBE16(x) ((uint16_t) ((((uint16_t)(x) & 0x00ff) << 8) | \
                                     (((uint16_t)(x) & 0xff00) >> 8)))
#endif
#ifndef SDL_SwapBE32
#define SDL_SwapBE32(x) ((uint32_t) ((((uint32_t)(x) & 0x000000ffu) << 24) | \
...
midi_file_t *MIDI_LoadFileMem(void *buf, size_t buflen)
{
    midi_file_t *file;
    MEMFILE *stream;
    ...
    if (stream == NULL)
    {
        fprintf(stderr, "MIDI_LoadFileMem: Failed to open memory stream\n");
        MIDI_FreeFile(file);
        return NULL;
    }
...
midi_file_t *MIDI_LoadFile(char *filename)
{
    ...
    // Read the whole file into memory, then parse it from there.
    ...
    got = fread(buf, 1, (size_t) len, stream);
    fclose(stream);
    ...
    file = MIDI_LoadFileMem(buf, (size_t) len);
