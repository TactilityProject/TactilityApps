...
// Writing the config was disabled along with the rest of the ORIGCODE blocks
// when doomgeneric was ported, which meant M_SaveDefaults() was a no-op and
// nothing set in the options menu ever persisted. The body needs nothing but
// stdio and the scantokey table defined above in this file, and the matching
// loader (LoadDefaultCollection) was already enabled, so it is turned back on
...
static void SetVariable(default_t *def, char *value)
{
    ...
    switch (def->type)
    {
        ...
        case DEFAULT_KEY:
            ...
            {
                // Not a DOS scancode. Vanilla zeroed anything outside the
                // table, but this port defines its own virtual keys above it -
                // KEY_STRAFE_L/R (0xa0/0xa1), KEY_USE (0xa2) and KEY_FIRE
                // (0xa3) - and those are the defaults for the fire, use and
                // strafe bindings. Zeroing them unbound fire and use entirely
                // as soon as a config file existed to read back. There is no
                // scancode to translate to, so keep the value as-is.
            }
...
// Reading the config was disabled along with the rest of the ORIGCODE blocks
// when doomgeneric was ported (as was SaveDefaultCollection above). With only
// the writer re-enabled the settings appeared not to persist at all: nothing
// was ever read back, so the built-in defaults were still in place at exit and
// were written straight over the file the previous run had saved.
//
// The body needs nothing beyond stdio and SearchCollection/SetVariable from
// this same file.
static void LoadDefaultCollection(default_collection_t *collection)
{
    ...
    while (!feof(f))
    {
        ...
        // from DOS text files)
        //
        // The cast matters: char is signed on this target, and passing a
        // negative value to isprint() is undefined behaviour (it is only
        // defined for unsigned char values and EOF). Any byte >= 0x80 - which
        // the chat macro strings can contain - would index the ctype table out
        // of bounds.
...
extern char doomEsp_savedir[128];

static char *GetDefaultConfigDir(void)
{
    char *result = (char *)malloc(sizeof(doomEsp_savedir));
